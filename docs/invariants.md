...
| The server reports every LAN origin it answers on at `/me`, and a client follows them when the paired address stops answering | `apps/server/src/http/routes/device.ts`, `packages/client-core/src/origin-failover.ts` | `apps/server/test/originFailover.test.ts` → *the addresses a phone is told about*; `packages/client-core/src/__tests__/origin-failover.test.ts` |
| Cross-origin requests are allowed only between this machine's own addresses — the certificate's SAN — and refused for anything else | `apps/server/src/index.ts` (`isOwnOrigin`) | `apps/server/test/originFailover.test.ts` → *cross-origin requests between this machine's own addresses* |
